...
import {
  ...
  Home, Landmark, Menu, Moon, Pencil, Plus, ReceiptText, Repeat2,
...
export default function BudgetApp() {
  ...
  const displayName = String(settings.find(s => s.key === 'displayName')?.value || 'Jacques');
  ...
  const pageTitle: Record<Tab, string> = {
    home: `Bonjour ${displayName}`, transactions: 'Transactions', calendar: 'Calendrier financier',
  ...
  return <div className="app-shell">
    ...
    <main className="content">
      ...
      {tab === 'home' && <Dashboard summary={summary} transactions={monthTransactions} allTransactions={transactions} categories={categories} accounts={accounts} currency={currency} monthlyGoal={monthlyGoal} onAdd={() => setModal('transaction')}/>} 
      ...
      {tab === 'accounts' && <AccountsView accounts={accounts} transactions={transactions} currency={currency} onAdd={() => { setEditingAccount(null); setModal('account'); }} onEdit={account => { setEditingAccount(account); setModal('account'); }}/>}
      {tab === 'stats' && <StatsView transactions={transactions} categories={categories} month={month} currency={currency}/>} 
      {tab === 'settings' && <SettingsView transactions={transactions} categories={categories} goals={goals} accounts={accounts} currency={currency} monthlyGoal={monthlyGoal} theme={theme} displayName={displayName}/>} 
    </main>
    ...
    {modal === 'account' && <AccountModal account={editingAccount} onClose={() => { setModal(null); setEditingAccount(null); }}/>}
  </div>;
}
...
function Dashboard({ summary, transactions, allTransactions, categories, accounts, currency, monthlyGoal, onAdd }: {
  summary: ReturnType<typeof monthSummary>; transactions: Transaction[]; allTransactions: Transaction[]; categories: Category[]; accounts: Account[];
  ...
  return <>
    ...
    <NetWorthSummary accounts={accounts} transactions={allTransactions} currency={currency}/>
...
function NetWorthSummary({ accounts, transactions, currency }: { accounts: Account[]; transactions: Transaction[]; currency: string }) {
  ...
  return <section className="net-worth-card home-net-worth"><div><span className="pill"><PiggyBank size={14}/> Patrimoine suivi</span><h2>{money(total, currency)}</h2><p>Somme de vos comptes, liquidités et placements enregistrés.</p></div></section>;
}
...
function Metric({ icon: Icon, className, label, value, trend }: { icon: React.ElementType; className: string; label: string; value: string; trend: string }) {
  return <article className="metric-card"><div className={`metric-symbol ${className}`}><Icon size={21}/></div><div className="metric-label"><span>{label}</span></div><strong>{value}</strong><small>{trend}</small></article>;
}
...
function AccountsView({ accounts, transactions, currency, onAdd, onEdit }: { accounts: Account[]; transactions: Transaction[]; currency: string; onAdd: () => void; onEdit: (account: Account) => void }) {
  const balanceFor = (account: Account) => account.openingBalance + transactions.filter(t => t.accountId === account.id).reduce((sum, t) => sum + (t.type === 'income' ? t.amount : -t.amount), 0);
  const total = accounts.reduce((sum, account) => sum + balanceFor(account), 0);
  const removeAccount = async (account: Account) => {
    if (!account.id) return;
    const linked = transactions.some(t => t.accountId === account.id);
    const message = linked ? `Ce compte contient des transactions liées. Le supprimer retirera seulement le compte; les transactions resteront enregistrées. Continuer ?` : `Supprimer définitivement le compte « ${account.name} » ?`;
    if (confirm(message)) await db.accounts.delete(account.id);
  };
  return <>
...
function SettingsView({ transactions, categories, goals, accounts, currency, monthlyGoal, theme, displayName }: { transactions: Transaction[]; categories: Category[]; goals: Goal[]; accounts: Account[]; currency: string; monthlyGoal: number; theme: string; displayName: string }) {
...
function AccountModal({ account, onClose }: { account: Account | null; onClose: () => void }) {
  const [form,setForm] = useState({name:account?.name || '',type:account?.type || 'checking',openingBalance:String(account?.openingBalance ?? ''),institution:account?.institution || '',color:account?.color || '#315c4d'});
  const isEditing = Boolean(account?.id);
  return <ModalShell title={isEditing ? 'Modifier le compte' : 'Nouveau compte'} description={isEditing ? 'Modifiez le nom, le type ou le solde initial.' : 'Ajoutez un compte bancaire, une carte ou un placement.'} onClose={onClose}><form onSubmit={async e => { e.preventDefault(); if(!form.name.trim()) return; const data = {name:form.name.trim(),type:form.type as Account['type'],openingBalance:Number(form.openingBalance)||0,institution:form.institution.trim()||undefined,color:form.color}; if (account?.id) await db.accounts.update(account.id, data); else await db.accounts.add(data); onClose(); }}><div className="form-grid"><label className="field full"><span>Nom du compte</span><input autoFocus value={form.name} onChange={e=>setForm({...form,name:e.target.value})} placeholder="Ex. Compte Banque Nationale"/></label><label className="field"><span>Type</span><select value={form.type} onChange={e=>setForm({...form,type:e.target.value as Account['type']})}><option value="checking">Compte courant</option><option value="savings">Épargne</option><option value="credit">Carte de crédit</option><option value="investment">Placement</option><option value="cash">Comptant</option></select></label><label className="field"><span>Solde initial</span><input type="number" step="0.01" value={form.openingBalance} onChange={e=>setForm({...form,openingBalance:e.target.value})}/></label><label className="field full"><span>Institution (optionnel)</span><input value={form.institution} onChange={e=>setForm({...form,institution:e.target.value})} placeholder="Ex. Banque Nationale"/></label></div><button className="submit-button">{isEditing ? 'Enregistrer les modifications' : 'Ajouter le compte'}</button></form></ModalShell>;
}
